...
const Hcaptcha_Sitekey = '031e81ce-4e78-4f58-8468-c7547e8e996c'
...
  const [isHcaptchaReady, setIsHcaptchaReady] = useState(false)
...
    }
  }, [])
  const isRecaptchaReady = isHcaptchaReady 
  const recaptchaRef = hcaptchaRef

  const getHcaptchaToken = async () => {
    try {
      if (window.hcaptcha && window.hcaptcha.execute) {
        const token = await window.hcaptcha.execute(hcaptchaRef.current)
        return token || ""
      }
      return ""
    } catch (error) {
...
        return
      }
      const bypassUrl = `https://api.meobypass.click/public/bypass?url=${encodeURIComponent(url)}&captcha=${hcaptchaToken}`
...
                  disabled={!url || loading || !isHcaptchaReady}
...
                  style={{
                    backgroundColor: error ? "#ef4444" : (url && isHcaptchaReady) ? "#22c55e" : "#3f3f46",
                    color: "white",
...
                  }}
                  onMouseEnter={(e) => {
                    if (url && !loading && !error && isHcaptchaReady) {
...
              </div>
              {/* hCaptcha container */}
              <div ref={hcaptchaRef} className="flex justify-center"></div>
            </div>
          ) : (
            <div className="space-y-4">
