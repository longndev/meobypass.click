...
export default function BypassPage() {
  ...
  const addToast = (message, type = 'info') => {
    ...
    // Clear existing timeout for the same message type if exists
    ...
    try {
      ...
      if (window.grecaptcha && window.grecaptcha.execute) {
        try {
          ...
          addToast('Recaptcha verification failed', 'error');
          setLoading(false);
          return;
        }
      } else {
        console.error('Recaptcha not loaded');
        addToast('Please wait for recaptcha to load', 'error');
        setLoading(false);
        return;
      }
